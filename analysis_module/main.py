...
async def prepare_trading(signals):
    ...
    try:
        ...
        print("[INFO] Preparing trading...")
        result = client.query(f"""
           SELECT token FROM userdetails WHERE strategy = '{strategy}' AND trading = 'true' AND trading_today = 'true'
        """)
        ...
        for signal in signals:
            for token in tokens:
                ...
                """
                result = client.query(f"""
                SELECT email, trading_today FROM userdetails WHERE token = '{token}'
                """)
                email = [row[0] for row in result.result_set]
                trading_today = [row[1] for row in result.result_set]
                
                if trading_today == 'true':
                    eligible = await eligible_user(email[0],token)
                    ...
                    print(f"{BLUE}[WARNING] {email}. not trading today Trade skipped.{RESET}")   
    except Exception as e:
        print(f"[ERROR] Error in prepare_trading: {e}")
        logging.error(f"Error in prepare_trading: {e}")
